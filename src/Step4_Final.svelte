<script lang="ts">
  ...
  let animationArea: HTMLDivElement = null;
  let topSize = 0;
  let bottomSize = 0;
  let frame = 0;
  ...
  const animationFrame = () => {
    ...
    const scrollTop = html.scrollTop - topSize;
    const maxScrollTop = animationArea.clientHeight - bottomSize;
    const scrollFraction = scrollTop / maxScrollTop;
    const frameIndex = Math.min(frameCount - 1, Math.ceil(scrollFraction * frameCount));
    requestAnimationFrame(() => {
      updateImage((frameIndex < 0 ? 0 : frameIndex));
    });
  };

  const onResize = () => {
    const top = animationArea.offsetTop;
    const bottom = animationArea.offsetTop + animationArea.offsetHeight;
    topSize = document.body.scrollTop + top;
    bottomSize = document.body.scrollHeight - bottom;
  }
...
<style lang="scss">
  .top-area, .bottom-area, .top-area2, .bottom-area2 {
    color: white;
    display: flex;
